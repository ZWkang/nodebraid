...
test('Dagre full layout supports the baseline graph shapes and self-loops', async () => {
  const alphaId = nodeId('alpha');
  const betaId = nodeId('beta');
  ...
  const [first, second, empty] = await Promise.all([
    dagreLayoutEngine.compute(input, {}, context),
    dagreLayoutEngine.compute(input, {}, context),
    dagreLayoutEngine.compute(emptyInput, {}, context),
  ]);

  expect({
    capabilities: dagreLayoutEngine.capabilities,
