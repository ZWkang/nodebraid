...
**What to build:** 让 NodeBraid 调用者可以创建一个隔离的空 Plugin Host，定义并安装一个不依赖 Runtime Service 的 Plugin，观察它完成 Activation 或显式失败，并安全结束 Plugin Installation 与整个 Host。Cordis 承担真实生命周期工作，但不会出现在 NodeBraid 的公开 interface 中。

**Blocked by:** None — can start immediately.

**Status:** resolved

- [x] 新增可独立发布和测试的 `@nodebraid/runtime-cordis` package，并精确依赖实现时 npm latest 的 Cordis 4 预发布版本。
- [x] 公开 NodeBraid 自己的 Plugin、Plugin Context、Plugin Installation、Plugin Host、Installation Snapshot 与结构性错误类型，不公开 Cordis Context、Fiber、Service 或 effect 类型。
...
- [x] 状态订阅只报告 NodeBraid Snapshot 变化，解除订阅幂等，同一状态期间 Snapshot 保持相同引用。
