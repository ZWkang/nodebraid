# NodeBraid Documentation Site

**Status:** ready-for-agent

## Problem Statement

NodeBraid 已经交付 15 个相互组合的 package，但外部 TypeScript 开发者目前只能从根 README、各 package README、目标架构、阶段性基线、领域术语和 ADR 中自行拼接产品全貌。这些材料面向不同读者，内容深度不一致，还混合了当前实现、目标设计和暂未交付能力。结果是开发者难以快速判断 NodeBraid 当前能解决什么问题、应从哪里开始、哪些 package 需要一起使用，以及哪些能力仍不可用。
...
此外，`@nodebraid/*` package 尚未公开发布。本仓库暂时没有已经生效、可安全复制的 npm 安装命令。若文档直接宣传 `bun add @nodebraid/core`，访问者会得到 registry 失败或未经验证的安装结果。

## Solution

建设一个中文优先的 NodeBraid Documentation Site，首要服务正在评估或采用 NodeBraid 的外部 TypeScript 开发者。站点将 NodeBraid 准确定位为可组合、Renderer-agnostic 的 headless flow canvas engine，介绍当前已经交付的 Plugin、Runtime Service、Provider 与 package 组合关系，不把目标架构或并行设计包装成可用功能。
...
首页的主要转化动作是运行一个真实 Quick Start。首发 Quick Start 只提供源码 checkout 流程，通过 `@nodebraid/core` 公共 facade 创建 Plugin Host、安装 Kernel Plugin、提交一个 Node，并正确等待 Activation 与释放 Host。所有安装区域必须明确 package 尚未公开发布，禁止展示尚未生效的 npm 安装命令。

站点通过一个顶层 `docs:check` seam 完成 Quick Start 执行和生产构建，并由仓库总检查复用。完整内容通过 GitHub Pages 发布，首发使用仓库子路径，未来可迁移 npm scope、英文内容和自定义域名。

## User Stories

1. As an external TypeScript developer, I want to understand NodeBraid's current product position from the homepage, so that I can decide whether it fits my canvas-engine needs.
2. As an external TypeScript developer, I want NodeBraid described as a headless canvas engine, so that I do not mistake it for a finished visual editor.
...
4. As an evaluator, I want a concise explanation of why NodeBraid is plugin-based and renderer-agnostic, so that I can understand its differentiating architecture without reading internal ADRs.
...
9. As an evaluator, I want source checkout instructions while packages are unpublished, so that I can run the correct project without relying on an unavailable npm package.
...
- Extend the default VitePress theme only where needed for a product-facing homepage, capability cards and NodeBraid-specific visual language. Keep documentation navigation, search, code blocks and responsive layout on the maintained default theme.
- Use a text-based NodeBraid mark in the first release. Do not invent a permanent logo or brand system without a separate design decision.
...
- State that this project's packages are not yet available under their declared npm names. Use source checkout instructions and never show `bun add @nodebraid/core` before the initial release is verified.
...
- Verify that no public page contains an npm installation command before the initial package release is available.
...
- Claiming, creating or modifying the `@nodebraid` npm scope.
...
- The `@nodebraid/*` packages have not yet been publicly released. This is a launch constraint, not a documentation-generation error, and must remain visible until publication succeeds.
