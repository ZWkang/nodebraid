---
title: Interaction
description: 把 Renderer Input 与 Hit Result 解释为 Selection、Drag、Pan 和 Zoom，同时保持稳定写入边界。
---
...
| 层次         | Package                                                    | 职责                                                      |
| ------------ | ---------------------------------------------------------- | --------------------------------------------------------- |
| 值契约       | [`@cflow/interaction-api`](/modules/interaction-api)       | Node Drag 与 Viewport Pan 的不可变 Projection             |
| Runtime 行为 | [`@cflow/plugin-interaction`](/modules/plugin-interaction) | Selection、multi-Node Drag、Pan、Wheel Zoom 与生命周期    |
...
首版不包含 box selection、edge connect、delete shortcut、snapping、pinch/touch 工具、HTML overlay、文本编辑、协作 presence、产品 UI 或通用 Tool Registry。SVG Provider 也不解释产品 Node 类型或数据。
