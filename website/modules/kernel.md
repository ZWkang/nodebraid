...
description: NodeBraid 的 Renderer-independent 图内核、同步 Transaction 与可逆 Change Set。
---
...
`@nodebraid/kernel` 是 NodeBraid 的权威 Document 所有者。它只处理 Node、Edge、Endpoint、关系索引与 revision，不知道 Plugin Host、Session、Renderer 或前端框架。

::: info 当前状态
已实现。纯 Kernel 与 Runtime adapter 已经分离，当前 `@nodebraid/plugin-kernel` 也已实现；“分离”不表示 Kernel Plugin 仍是未来规划。
:::

## 解决什么问题

画布写入需要同时满足三件事：图结构始终有效、一次操作要么完整提交要么完全回滚、读取者不能把旧 Snapshot 与新 Query 混用。`@nodebraid/kernel` 把这些规则收敛到一个小而严格的同步事务边界中。
...
`@nodebraid/kernel` 的唯一 workspace 依赖是叶子包 `@nodebraid/diagnostics`，用于共享结构性错误契约。它不依赖 `@nodebraid/core`，也不依赖任何 Runtime 或 Renderer package。

在 Canvas Runtime 中，不要把裸 `CanvasKernel` 当作全局对象传递；由 [`@nodebraid/plugin-kernel`](/modules/plugin-kernel) 把它封装成窄 `KernelService`，并拥有 Commit Observer 与 Activation 生命周期。
...
- Snapshot、Query 结果和 NodeBraid 自有值按规范 ID 顺序且不可变；这个顺序只保证确定性，不代表 z-index。

最终图必须满足：Node position 是有限数值；可选 size 是有限且非负；parent 必须存在且不能成环；每个 Edge Endpoint 必须引用现存 Node。

## 限制与非目标

- 不提供 Plugin Installation、Runtime Service、Commit Observer 或资源释放；这些属于 `@nodebraid/plugin-kernel`。
...
- [公共导出](https://github.com/ZWkang/nodebraid/blob/main/packages/kernel/src/index.ts)只包含 Kernel contract、ID helper 与结构性错误。
- [Kernel 行为测试](https://github.com/ZWkang/nodebraid/blob/main/packages/kernel/tests/index.test.ts)覆盖原子提交、回滚、关系查询、最终图校验、净零事务和 Change Set replay。
- [错误与类型测试](https://github.com/ZWkang/nodebraid/tree/main/packages/kernel/tests)验证稳定 error identity、readonly View/Commit 与同步 Transaction 边界。
