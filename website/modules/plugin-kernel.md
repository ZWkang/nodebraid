...
`@cflow/plugin-kernel` 是已经交付的 Kernel Runtime adapter。它为每次 Plugin Activation 创建一份独立 Kernel，并让其他 Plugin 只通过 `KernelService` 使用权威 Document。

::: info 当前状态
已实现，并已由 `@cflow/core` facade 重导出。它不是未来占位模块，也不会把裸 `CanvasKernel` 暴露给 Runtime Consumer。
:::
...
- `kernelPluginDiagnosticEvents`：公开稳定的 `cflow.plugin.kernel.observer.fault` 事件名。

## 依赖与组合

`@cflow/plugin-kernel` 直接依赖：

- `@cflow/kernel`：创建并操作纯 Kernel。
- `@cflow/runtime-cordis`：CFlow-owned Plugin Host、Service Token 与 Activation seam。
- `@cflow/diagnostics`：Observer fault 的共享诊断契约。

它不依赖 `@cflow/core`。Consumer 通过局部 Service Binding 获取 `KernelService`；[`@cflow/plugin-session`](/modules/plugin-session) 就是一个静态依赖 `kernelService` 的官方 Consumer。
...
- [公共导出](https://github.com/ZWkang/cflow/blob/main/packages/plugin-kernel/src/index.ts)确认当前模块已真实提供 Plugin、Service Token、错误与诊断事件。
- [Runtime 实现](https://github.com/ZWkang/cflow/blob/main/packages/plugin-kernel/src/kernel-plugin.ts)显示每次 Activation 创建纯 Kernel，并在 adapter 内拥有 Commit queue 与 Observer。
- [公开 seam 行为测试](https://github.com/ZWkang/cflow/blob/main/packages/plugin-kernel/tests/index.test.ts)覆盖 revision-zero Service、净变化过滤、重入顺序、Observer fault 与依赖恢复后的新 Kernel。
- ADR-0013 明确 `@cflow/plugin-kernel` 是已经选定的窄 Kernel Runtime Service seam。
